...
0x03  extended-dict tokenized, then deflate-raw compressed
0x04  extended-dict tokenized only
```
...
- **Core tokens** — 250 one-byte codes (0x01..0xFA): schemes, TLDs, common
  ...
  **append-only**: additions may only be appended, never reordered, so old
  links always decode.
- **Extended tokens** — full domains from the Cisco Umbrella top-1000 plus a
  curated Nordic list, encoded as 3 bytes: `0xFF (0x80|hi) lo`. Only emitted
  when they beat the core tokens, so links that don't need them still use
  flags 1/2 and stay readable by older pages. The extended table is
  regenerated by `scripts/gen-dict.mjs` from `data/top-1000-domains.txt`.

v2 streams (flags 3/4) escape literal bytes as `0xFF b` (0x01..0x7F) or
`0xFF 0x00 b` (NUL and high bytes). Every extended token carries a ≥0x80
byte, so an old page misdecoding a v2 stream produces an invalid URL and
fails loudly instead of silently landing on the wrong site.

Plain (`u1.`) links use the same scheme via flag bits: bit4 = dict-tokenized,
bit5 = v2/extended stream (decode with the v2 rules only when bit5 is set).
