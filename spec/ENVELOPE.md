...
The dictionary has two tiers, both embedded in the page (never negotiated):
...
| tier | flags (encrypted) | u1 bits | literal encoding |
|---|---|---|---|
...
Every extended token carries a ≥0x80 byte and every v3 run starts with
...
Plain (`u1.`) links use the same tiers via flag bits: bit4 = dict-tokenized,
bit5 = v2, bit6 = v3.
