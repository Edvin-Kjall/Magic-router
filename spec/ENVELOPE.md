...
0x07  deep-dict tokenized, then deflate-raw compressed
0x08  deep-dict tokenized only
```
...
- **Deep tokens** — a downloadable dictionary (`/deep-v1.json.gz`, fetched
  once and cached by the browser): the worldwide top-10k domains, the EFF
  common-words list, and curated tech/AI terms. Same 3-byte encoding, but
  the codes index the deep table. Links that use it carry payload flags
  7/8 (encrypted) or the `u2.` prefix (plain); the opener pays one
  dictionary download on first use — the deliberate 3–5 s trade for the
  smallest possible links. The table is frozen per version: `deep-v2`
  will be a new asset and link prefix.

Stream tiers (chosen by the encoder, recorded in the flag byte):

| tier | flags (encrypted) | u1/u2 bits | literal encoding |
|---|---|---|---|
| legacy | 1/2 | 4 | `0xFF b` — one literal byte |
| v2 | 3/4 | 4+5 | + high-byte literals `0xFF 0x00 b` |
| v3 | 5/6 | 4+5+6 | + literal runs `0xFF 0x00 n <n bytes>` — long unknown words cost n+2 instead of 2n |
| deep | 7/8 | 4+5+6+7 (`u2.`) | v3 grammar, `0xFF hi≥0x80 lo` indexes the deep table |
...
dictionary tiers (bit4 = dict, bit5 = v2, bit6 = v3) and `u2.` for the
deep tier (bits 4+5+6+7).
